/*
 * Tibia 8.40 Level Spy
 * by Evremonde
 * Last updated on 03/12/2009
 */
...
void doTrayIconCreate(HWND hwnd)
{
    ...
    strcpy(trayIcon.szTip, "Tibia 8.40 Level Spy");
...
void doTrayMenuCreate(HWND hwnd)
{
    ...
    AppendMenu(trayMenu, MF_STRING, ID_TRAY_MENU_NULL,          "Tibia 8.40 Level Spy");
...
void doLoadIni()
{
    ...
    // level spy
    VK_HOTKEY_LEVELSPY = GetPrivateProfileInt("Hotkeys", "LevelSpy", VK_HOTKEY_LEVELSPY, "./tibia840levelspy.ini");
    VK_HOTKEY_LEVELSPY_UP = GetPrivateProfileInt("Hotkeys", "LevelSpyUp", VK_HOTKEY_LEVELSPY_UP, "./tibia840levelspy.ini");
    VK_HOTKEY_LEVELSPY_DOWN = GetPrivateProfileInt("Hotkeys", "LevelSpyDown", VK_HOTKEY_LEVELSPY_DOWN, "./tibia840levelspy.ini");

    // name spy
    VK_HOTKEY_NAMESPY = GetPrivateProfileInt("Hotkeys", "NameSpy", VK_HOTKEY_NAMESPY, "./tibia840levelspy.ini");

    // full light
    VK_HOTKEY_LIGHT = GetPrivateProfileInt("Hotkeys", "FullLight", VK_HOTKEY_LIGHT, "./tibia840levelspy.ini");

    // show invisible creatures
    VK_HOTKEY_INVISIBLE = GetPrivateProfileInt("Hotkeys", "ShowInvisibleCreatures", VK_HOTKEY_INVISIBLE, "./tibia840levelspy.ini");

    // hide trees
    VK_HOTKEY_TREES = GetPrivateProfileInt("Hotkeys", "HideTrees", VK_HOTKEY_TREES, "./tibia840levelspy.ini");

    /* options */

    // name spy
    if (GetPrivateProfileInt("Options", "NameSpy", 0, "./tibia840levelspy.ini") == 1)
        doNameSpyOn();

    // full light
    if (GetPrivateProfileInt("Options", "FullLight", 0, "./tibia840levelspy.ini") == 1)
        doFullLightOn();

    // show invisible creatures
    if (GetPrivateProfileInt("Options", "ShowInvisibleCreatures", 0, "./tibia840levelspy.ini") == 1)
        doShowInvisibleCreaturesOn();

    // hide trees
    if (GetPrivateProfileInt("Options", "HideTrees", 0, "./tibia840levelspy.ini") == 1)
        doHideTreesOn();

    // hotkeys
    if (GetPrivateProfileInt("Options", "Hotkeys", 1, "./tibia840levelspy.ini") == 0)
        bHotkeys = 0;
    else
        bHotkeys = 1;
}
...
void onCreate(HWND hwnd)
{
    ...
    // loaded
    setStatusbarText("Tibia 8.40 Level Spy loaded!");
}
...
void onDestroy(HWND hwnd)
{
    ...
    // unloaded
    setStatusbarText("Tibia 8.40 Level Spy unloaded!");
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR cmdline, int cmdshow)
{
    ...
    HWND applicationWindow = FindWindow(0, "Tibia 8.40 Level Spy");
    ...
    wc.lpszMenuName  = 0;
    wc.lpszClassName = "tibia840levelspy";
    ...
    hwnd = CreateWindowEx(
        ...
        "tibia840levelspy", "Tibia 8.40 Level Spy", // window class and title
