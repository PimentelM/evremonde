...
int outfitMonster	= OUTFIT_INVALID_1;
...
void toggleShowNames()
{
	if (bShowNames)
	{
		Tibia.setShowNames(false);

		::CheckMenuItem(botMenu, mnuShowNames, MF_UNCHECKED);

		bShowNames = false;
		Tibia.setStatusbar("Show Names: Off");
	}
	else
	{
		Tibia.setShowNames(true);

		::CheckMenuItem(botMenu, mnuShowNames, MF_CHECKED);

		bShowNames = true;
		Tibia.setStatusbar("Show Names: On");
	}
}
...
{
	Tibia.doXRay();
	Tibia.setStatusbar("X-Ray!");
}

void doOutfitMonsterNext()
{
	// hand-written

	outfitMonster++;

	// filter out invalid outfits and human outfits
	switch (outfitMonster)
	{
        case OUTFIT_INVALID_1:
        case OUTFIT_INVALID_2:
		    outfitMonster++;
		    break;
		    
        case OUTFIT_INVALID_3:
		case OUTFIT_INVALID_4:
			outfitMonster = OUTFIT_INVALID_4 + 1;
			break;
			
		case OUTFIT_INVALID_5:
			outfitMonster = OUTFIT_INVALID_1 + 1;
			break;
			
		case OUTFIT_FEMALE_OLD:
			outfitMonster = OUTFIT_FEMALE_PREMIUM_11 + 1;
			break;
	}

	Tibia.setPlayerOutfit(static_cast<Outfit_t>(outfitMonster));
	
	std::stringstream buffer;
	buffer << "Monster+" << " (" << outfitMonster << ")";
	Tibia.setStatusbar(buffer.str().c_str());
}

void doOutfitMonsterPrev()
{
	// hand-written
	...
	switch (outfitMonster)
	{
		case OUTFIT_INVISIBLE:
        case OUTFIT_INVALID_1:
		    outfitMonster = OUTFIT_INVALID_5 - 1;
		    break;

        case OUTFIT_INVALID_2:
			outfitMonster = OUTFIT_INVALID_2 - 1;
			break;

		case OUTFIT_INVALID_3:
        case OUTFIT_INVALID_4:
			outfitMonster = OUTFIT_INVALID_3 - 1;
			break;
			
		case OUTFIT_FEMALE_PREMIUM_11:
			outfitMonster = OUTFIT_FEMALE_OLD - 1;
			break;
	}
...
void doOutfitMonsterRandom()
{
	// hand-written
	
	// random range = rand() % (MAX - MIN + 1) + MIN;

	int outfitSelect = rand() % (2 - 1 + 1) + 1;
	switch (outfitSelect)
	{
		case 1:
			outfitMonster = rand() % ((OUTFIT_INVALID_2 - 1) - (OUTFIT_INVALID_1 + 1) + 1) + (OUTFIT_INVALID_1 + 1);
			break;
			
		case 2:
			outfitMonster = rand() % ((OUTFIT_INVALID_5 - 1) - (OUTFIT_INVALID_4 + 1) + 1) + (OUTFIT_INVALID_4 + 1);
			break;
	}
...
{
    bufferHotkeys
		<< "INSERT\t\tToggle outfit addon"							<< std::endl
		<< "DELETE\t\tApply x-ray to last seen tile"				<< std::endl
		<< "SHIFT+DELETE\tToggle show names through floor"			<< std::endl
		<< "HOME\t\tShow EXP needed to level up"					<< std::endl
		<< "SHIFT+HOME\tShow ID of last seen tile or item"			<< std::endl
		<< "END\t\tShow evrebot menu..."							<< std::endl
		<< "SHIFT+END\tView creatures..."							<< std::endl
		<< "PAGEUP\t\tToggle light radius"							<< std::endl
		<< "PAGEDN\t\tToggle click (fishing, roping, runes)"		<< std::endl
		<< "SHIFT+PAGEDN\tToggle automatic mouse clicking"			<< std::endl
		<< "SCROLL LOCK\tDo next monster outfit"					<< std::endl
		<< "SHIFT+SCROLL\tDo previous monster outfit"				<< std::endl
		<< "PAUSE\t\tToggle change trees to bushes"					<< std::endl
		<< "SHIFT+PAUSE\tToggle show water w/ fish"					<< std::endl
		<< "MOUSE3\t\tDo fast drag to location"						<< std::endl
...
void traySetTip(std::string trayText)
{
	// set tray tip text
	strcpy(trayData.szTip, trayText.c_str());
...
{
	// bot was opened before tibia was opened
	if (Tibia.Trainer.hwnd == 0)
	{
		// find tibia window
		...
		if (tibiaWindow)
		{
			...
			Tibia.Trainer.setProcess(tibiaWindow);

			// set tibia to low cpu usage
			::SetPriorityClass(Tibia.Trainer.processHandle, Tibia.Trainer.PRIORITY_BELOW_NORMAL);
		}
	}
	else
	{
...
void timerTrayTip()
{
	...
	if (Tibia.isInGame())
	{
		// update tray tip to include player's name
		std::string trayTip = "evrebot\n" + Tibia.getPlayerName();
		traySetTip(trayTip);
	}
	else
	{
...
void timerHotkeys(HWND hwnd)
{
	...
	// show tray menu OR view creatures
	...
	// toggle click OR toggle automatic mouse clicking
	...
	// do monster outfit next OR do monster outfit prev
	...
	// toggle trees to bushes OR toggle fishing water
...
void onCreate(HWND hwnd)
{
    ...
    ::SetTimer(hwnd, TMR_TRAYTIP,			1000, 0);
...
void onCommand(HWND hwnd, int command, int notifyCode, HWND hwndItem)
{
	switch (command) 
	{
		...
		// outfit monster
        case mnuOutfitMonsterRandom:
			doOutfitMonsterRandom();
			break;
			
        case mnuOutfitMonsterNext:
			doOutfitMonsterNext();
			break;
			
        case mnuOutfitMonsterPrev:
			doOutfitMonsterPrev();
			break;

		// outfit other
		case mnuOutfitOtherGamemaster:
			Tibia.setPlayerOutfit(OUTFIT_GM);
			break;

		case mnuOutfitOtherElf:
			Tibia.setPlayerOutfit(OUTFIT_ELF);
			break;

		case mnuOutfitOtherDwarf:
			Tibia.setPlayerOutfit(OUTFIT_DWARF);
			break;

		case mnuOutfitOtherCult:
			Tibia.setPlayerOutfit(OUTFIT_CULT);
			break;

		case mnuOutfitOtherFrog:
			Tibia.setPlayerOutfit(OUTFIT_FROG);
			break;

		case mnuOutfitOtherFerumbras:
			Tibia.setPlayerOutfit(OUTFIT_FERUMBRAS);
			break;
