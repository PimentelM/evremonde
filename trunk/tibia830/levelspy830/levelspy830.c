...
// level spy
#define LEVELSPY_NOP	0x004D09E0 // writeNops 2 bytes // 004D09E0 | 2BC1 | SUB EAX,ECX
#define LEVELSPY_ABOVE	0x004D09DC // writeBytes 1 byte // 004D09DB | B8 07 | MOV EAX,7
#define LEVELSPY_BELOW	0x004D09E4 // writeBytes 1 byte // 004D09E3 | B8 02 | MOV EAX,2
// 004D09D0 | 8B0D 905B6300 | MOV ECX,DWORD PTR DS:[635B90]
// 004D09D6 | 83F9 07       | CMP ECX,7
// 004D09D9 | 7F 08         | JG SHORT Tibia.004D09E3
// 004D09DB | B8 07000000   | MOV EAX,7
// 004D09E0 | 2BC1          | SUB EAX,ECX
// 004D09E2 | C3            | RETN
// 004D09E3 | B8 02000000   | MOV EAX,2
// 004D09E8 | C3            | RETN

// name spy
#define NAMESPY_NOP		0x004EBD69 // writeNops 2 bytes // 004EBD69 | 75 4A | JNZ SHORT Tibia.004EBDB5
#define NAMESPY_NOP2	0x004EBD73 // writeNops 2 bytes // 004EBD73 | 75 40 | JNZ SHORT Tibia.004EBDB5
// 004EBD67 | 3901        | CMP DWORD PTR DS:[ECX],EAX
// 004EBD69 | 75 4A       | JNZ SHORT Tibia.004EBDB5
// 004EBD6B | E8 604CFEFF | CALL Tibia.004D09D0
// 004EBD70 | 3945 18     | CMP DWORD PTR SS:[EBP+18],EAX
// 004EBD73 | 75 40       | JNZ SHORT Tibia.004EBDB5

// show invisible creatures
#define INVISIBLE1		0x0045E003 // writeBytes 1 byte // 0045E003 | 72 0A | JB SHORT 0045E00F
// 0045DFFE | 3D 00000040 | CMP EAX,40000000
// 0045E003 | 72 0A       | JB SHORT Tibia.0045E00F

#define INVISIBLE2		0x004EB035 // writeBytes 1 byte // 004EB035 | 75 09 | JNZ SHORT Tibia.004EB040
// 004EB033 | 84C0  | TEST AL,AL
// 004EB035 | 75 09 | JNZ SHORT Tibia.004EB040

// full light
#define LIGHT_NOP		0x004E4519 // writeNops 2 bytes // 004E4519 | 7E 05 | JLE SHORT Tibia.004E4520
#define LIGHT_AMOUNT	0x004E451C // writeBytes 1 byte // 004E451B | BB 80000000 | MOV EBX,80
// 004E4509 | 8BD8          | MOV EBX,EAX
// 004E450B | 85DB          | TEST EBX,EBX
// 004E450D | 7D 04         | JGE SHORT Tibia.004E4513
// 004E450F | 33DB          | XOR EBX,EBX
// 004E4511 | EB 0D         | JMP SHORT Tibia.004E4520
// 004E4513 | 81FB 80000000 | CMP EBX,80
// 004E4519 | 7E 05         | JLE SHORT Tibia.004E4520
// 004E451B | BB 80000000   | MOV EBX,80
// 004E4520 | 8D4424 20     | LEA EAX,DWORD PTR SS:[ESP+20]
// 004E4524 | 50            | PUSH EAX
// 004E4525 | E8 76C7FEFF   | CALL Tibia.004D0CA0

// player
#define PLAYER_Z		0x00635B90 // 004D09D0 | 8B0D 905B6300 | MOV ECX,DWORD PTR DS:[635B90]
// 004D09D0 | 8B0D 905B6300 | MOV ECX,DWORD PTR DS:[635B90]
// 004D09D6 | 83F9 07       | CMP ECX,7
...
// z-axis
const int Z_AXIS_DEFAULT = 7; // default ground level // 004D09D6 | 83F9 07 | CMP ECX,7
// 004D09D0 | 8B0D 905B6300 | MOV ECX,DWORD PTR DS:[635B90]
// 004D09D6 | 83F9 07       | CMP ECX,7
...
// level spy
const int LEVELSPY_NOP_DEFAULT = 49451; // integer 2 bytes // 004D09E0 | 2BC1 | SUB EAX,ECX

const int LEVELSPY_ABOVE_DEFAULT = 7; // integer 1 byte // 004D09DB | B8 07 | MOV EAX,7
const int LEVELSPY_BELOW_DEFAULT = 2; // integer 1 byte // 004D09E3 | B8 02 | MOV EAX,2

const int LEVELSPY_MIN = 0;
const int LEVELSPY_MAX = 7;

// name spy
const int NAMESPY_NOP_DEFAULT = 19061; // integer 2 bytes // 004EBD69 | 75 4A | JNZ SHORT Tibia.004EBDB5
const int NAMESPY_NOP2_DEFAULT = 16501; // integer 2 bytes // 004EBD73 | 75 40 | JNZ SHORT Tibia.004EBDB5

// show invisible creatures
const int INVISIBLE1_DEFAULT = 114; // integer 1 byte // 0045E003 | 72 0A | JB SHORT 0045E00F
const int INVISIBLE2_DEFAULT = 117; // integer 1 byte // 004EB035 | 75 09 | JNZ SHORT Tibia.004EB040

const int INVISIBLE_PATCH = 235; // integer 1 byte // 235 = EB = JMP // change JB and JNZ to JMP

// full light
const int LIGHT_NOP_DEFAULT = 1406; // integer 2 bytes // 004E4519 | 7E 05 | JLE SHORT Tibia.004E4520

const int LIGHT_AMOUNT_DEFAULT = 128; // integer 1 byte // 004E451B | BB 80000000 | MOV EBX,80
const int LIGHT_AMOUNT_FULL = 255; // integer 1 byte // 004E451B | BB 80000000 | MOV EBX,80
...
void doTrayIconCreate(HWND hwnd)
{
	...
	strcpy(trayIcon.szTip, "Level Spy - Tibia 8.30");
...
void doTrayMenuCreate(HWND hwnd)
{
	...
	AppendMenu(trayMenu, MF_STRING, ID_TRAY_MENU_NULL,				"Level Spy - Tibia 8.30");
	...
	AppendMenu(trayMenu, MF_STRING, ID_TRAY_MENU_NUMPAD_HOTKEYS,	"Numpad Hotkeys");
	AppendMenu(trayMenu, MF_SEPARATOR, 0, 0);
...
void onCreate(HWND hwnd)
{
	...
	// loaded
	setStatusbarText("Level Spy for Tibia 8.30 loaded!");
}
...
void onDestroy(HWND hwnd)
{
	...
	// unloaded
	setStatusbarText("Level Spy for Tibia 8.30 unloaded!");
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR cmdline, int cmdshow)
{
	...
	HWND applicationWindow = FindWindow(0, "Level Spy - Tibia 8.30");
	...
	hwnd = CreateWindowEx(
		...
		"levelspy", "Level Spy - Tibia 8.30", // window class and title
