...
{
	// unprotect
	DWORD oldProtection = Trainer.setProtection(RSA_KEY, PAGE_READWRITE, RSA_KEY_SIZE);

	// set rsa key
	Trainer.writeChar(RSA_KEY, key, RSA_KEY_SIZE);

	// reprotect
	Trainer.setProtection(RSA_KEY, oldProtection, RSA_KEY_SIZE);
}

void CTibia::setFpsLimit(int fps)
{
	double newFps = static_cast<double>(1000 / fps);
