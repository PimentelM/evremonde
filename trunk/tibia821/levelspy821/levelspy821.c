...
// statusbar
#define STATUSBAR_TEXT	0x0077EA58
#define STATUSBAR_TIMER	0x0077EA54 // timer for showing text

// level spy
#define LEVELSPY_NOP	0x004CE030 // writeNops 2 bytes // 004CE030 | 2BC1 | SUB EAX,ECX
#define LEVELSPY_ABOVE	0x004CE02C // writeBytes 1 byte // 004CE02B | B8 07 | MOV EAX,7
#define LEVELSPY_BELOW	0x004CE034 // writeBytes 1 byte // 004CE033 | B8 02 | MOV EAX,2
// 004CE020 | 8B0D F8FA6200 | MOV ECX,DWORD PTR DS:[62FAF8]
// 004CE026 | 83F9 07       | CMP ECX,7
// 004CE029 | 7F 08         | JG SHORT Tibia.004CE033
// 004CE02B | B8 07000000   | MOV EAX,7
// 004CE030 | 2BC1          | SUB EAX,ECX
// 004CE032 | C3            | RETN
// 004CE033 | B8 02000000   | MOV EAX,2
// 004CE038 | C3            | RETN

// name spy
#define NAMESPY_NOP		0x004E9539 // writeNops 2 bytes // 004E9539 | 75 4A | JNZ SHORT 004E9585
#define NAMESPY_NOP2	0x004E9543 // writeNops 2 bytes // 004E9543 | 75 40 | JNZ SHORT 004E9585
// 004E9537 | 3901        | CMP DWORD PTR DS:[ECX],EAX
// 004E9539 | 75 4A       | JNZ SHORT Tibia.004E9585
// 004E953B | E8 E04AFEFF | CALL Tibia.004CE020
// 004E9540 | 3945 18     | CMP DWORD PTR SS:[EBP+18],EAX
// 004E9543 | 75 40       | JNZ SHORT Tibia.004E9585

// show invisible creatures
#define INVISIBLE1		0x0045C003 // writeBytes 1 byte // 0045C003 | 72 0A | JB SHORT 0045C00F
// 0045BFFE | 3D 00000040 | CMP EAX,40000000
// 0045C003 | 72 0A       | JB SHORT Tibia.0045C00F

#define INVISIBLE2		0x004E8805 // writeBytes 1 byte // 004E8805 | 75 09 | JNZ SHORT 004E8810
// 004E8803 | 84C0  | TEST AL,AL
// 004E8805 | 75 09 | JNZ SHORT Tibia.004E8810

// full light
#define LIGHT_NOP		0x004E1CA9 // writeNops 2 bytes // 004E1CA9 | 7E 05 | JLE SHORT 004E1CB0
#define LIGHT_AMOUNT	0x004E1CAC // writeBytes 1 byte // 004E1CAB | BB 80 | MOV EBX,80
// 004E1C99 | 8BD8          | MOV EBX,EAX
// 004E1C9B | 85DB          | TEST EBX,EBX
// 004E1C9D | 7D 04         | JGE SHORT Tibia.004E1CA3
// 004E1C9F | 33DB          | XOR EBX,EBX
// 004E1CA1 | EB 0D         | JMP SHORT Tibia.004E1CB0
// 004E1CA3 | 81FB 80000000 | CMP EBX,80
// 004E1CA9 | 7E 05         | JLE SHORT Tibia.004E1CB0
// 004E1CAB | BB 80000000   | MOV EBX,80
// 004E1CB0 | 8D4424 20     | LEA EAX,DWORD PTR SS:[ESP+20]
// 004E1CB4 | 50            | PUSH EAX
// 004E1CB5 | E8 36C6FEFF   | CALL Tibia.004CE2F0

// player
#define PLAYER_Z		0x0062FAF8 // 004CE020 | MOV ECX,DWORD PTR DS:[62FAF8]
// 004CE020 | 8B0D F8FA6200 | MOV ECX,DWORD PTR DS:[62FAF8]
// 004CE026 | 83F9 07       | CMP ECX,7
...
// z-axis
const int Z_AXIS_DEFAULT = 7; // default ground level // 004CE026 | 83F9 07 | CMP ECX,7
// 004CE020 | 8B0D F8FA6200 | MOV ECX,DWORD PTR DS:[62FAF8]
// 004CE026 | 83F9 07       | CMP ECX,7
...
// level spy
const int LEVELSPY_NOP_DEFAULT = 49451; // integer 2 bytes // 004CE030 | 2BC1 | SUB EAX,ECX

const int LEVELSPY_ABOVE_DEFAULT = 7; // integer 1 byte // 004CE02B | B8 07 | MOV EAX,7
const int LEVELSPY_BELOW_DEFAULT = 2; // integer 1 byte // 004CE033 | B8 02 | MOV EAX,2

const int LEVELSPY_MIN = 0;
const int LEVELSPY_MAX = 7;

// name spy
const int NAMESPY_NOP_DEFAULT = 19061; // integer 2 bytes // 004E9539 | 75 4A | JNZ SHORT 004E9585
const int NAMESPY_NOP2_DEFAULT = 16501; // integer 2 bytes // 004E9543 | 75 40 | JNZ SHORT 004E9585

// show invisible creatures
const int INVISIBLE1_DEFAULT = 114; // integer 1 byte // 0045C003 | 72 0A | JB SHORT 0045C00F
const int INVISIBLE2_DEFAULT = 117; // integer 1 byte // 004E8805 | 75 09 | JNZ SHORT 004E8810

const int INVISIBLE_PATCH = 235; // integer 1 byte // 235 = EB = JMP // change JB and JNZ to JMP

// full light
const int LIGHT_NOP_DEFAULT = 1406; // integer 2 bytes // 004E1CA9 | 7E 05 | JLE SHORT 004E1CB0

const int LIGHT_AMOUNT_DEFAULT = 128; // integer 1 byte // 004E1CAB | BB 80 | MOV EBX,80
const int LIGHT_AMOUNT_FULL = 255; // integer 1 byte // 004E1CAB | BB 80 | MOV EBX,80
...
void doTrayIconCreate(HWND hwnd)
{
	...
	strcpy(trayIcon.szTip, "Level Spy - Tibia 8.21");
...
void doTrayMenuCreate(HWND hwnd)
{
	...
	AppendMenu(trayMenu, MF_STRING, ID_TRAY_MENU_NULL,				"Level Spy - Tibia 8.21");
...
void onCreate(HWND hwnd)
{
	...
	// loaded
	setStatusbarText("Level Spy for Tibia 8.21 loaded!");
}
...
void onDestroy(HWND hwnd)
{
	...
	// unloaded
	setStatusbarText("Level Spy for Tibia 8.21 unloaded!");
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR cmdline, int cmdshow)
{
	...
	HWND applicationWindow = FindWindow(0, "Level Spy - Tibia 8.21");
	...
	hwnd = CreateWindowEx(
		...
		"levelspy", "Level Spy - Tibia 8.21", // window class and title
