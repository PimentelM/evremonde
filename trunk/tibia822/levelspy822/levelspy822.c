...
// level spy
#define LEVELSPY_NOP	0x004CE270 // writeNops 2 bytes // 004CE270 | 2BC1 | SUB EAX,ECX
#define LEVELSPY_ABOVE	0x004CE26C // writeBytes 1 byte // 004CE26B | B8 07 | MOV EAX,7
#define LEVELSPY_BELOW	0x004CE274 // writeBytes 1 byte // 004CE273 | B8 02 | MOV EAX,2
// 004CE260 | 8B0D F81A6300 | MOV ECX,DWORD PTR DS:[631AF8]
// 004CE266 | 83F9 07       | CMP ECX,7
// 004CE269 | 7F 08         | JG SHORT Tibia.004CE273
// 004CE26B | B8 07000000   | MOV EAX,7
// 004CE270 | 2BC1          | SUB EAX,ECX
// 004CE272 | C3            | RETN
// 004CE273 | B8 02000000   | MOV EAX,2
// 004CE278 | C3            | RETN

// name spy
#define NAMESPY_NOP		0x004E95F9 // writeNops 2 bytes // 004E95F9 | 75 4A | JNZ SHORT 004E9645
#define NAMESPY_NOP2	0x004E9603 // writeNops 2 bytes // 004E9603 | 75 40 | JNZ SHORT 004E9645
// 004E95F7 | 3901        | CMP DWORD PTR DS:[ECX],EAX
// 004E95F9 | 75 4A       | JNZ SHORT Tibia.004E9645
// 004E95FB | E8 604CFEFF | CALL Tibia.004CE260
// 004E9600 | 3945 18     | CMP DWORD PTR SS:[EBP+18],EAX
// 004E9603 | 75 40       | JNZ SHORT Tibia.004E9645

// show invisible creatures
#define INVISIBLE1		0x0045BF63 // writeBytes 1 byte // 0045BF63 | 72 0A | JB SHORT 0045BF6F
// 0045BF5E | 3D 00000040 | CMP EAX,40000000
// 0045BF63 | 72 0A       | JB SHORT Tibia.0045BF6F

#define INVISIBLE2		0x004E88C5 // writeBytes 1 byte // 004E88C5 | 75 09 | JNZ SHORT 004E88D0
// 004E88C3 | 84C0  | TEST AL,AL
// 004E88C5 | 75 09 | JNZ SHORT Tibia.004E88D0

// full light
#define LIGHT_NOP		0x004E1DA9 // writeNops 2 bytes // 004E1DA9 | 7E 05 | JLE SHORT 004E1DB0
#define LIGHT_AMOUNT	0x004E1DAC // writeBytes 1 byte // 004E1DAB | BB 80 | MOV EBX,80
// 004E1D99 | 8BD8          | MOV EBX,EAX
// 004E1D9B | 85DB          | TEST EBX,EBX
// 004E1D9D | 7D 04         | JGE SHORT Tibia.004E1DA3
// 004E1D9F | 33DB          | XOR EBX,EBX
// 004E1DA1 | EB 0D         | JMP SHORT Tibia.004E1DB0
// 004E1DA3 | 81FB 80000000 | CMP EBX,80
// 004E1DA9 | 7E 05         | JLE SHORT Tibia.004E1DB0
// 004E1DAB | BB 80000000   | MOV EBX,80
// 004E1DB0 | 8D4424 20     | LEA EAX,DWORD PTR SS:[ESP+20]
// 004E1DB4 | 50            | PUSH EAX
// 004E1DB5 | E8 76C7FEFF   | CALL Tibia.004CE530

// player
#define PLAYER_Z		0x00631AF8 // 004CE260 | MOV ECX,DWORD PTR DS:[631AF8]
// 004CE260 | 8B0D F81A6300 | MOV ECX,DWORD PTR DS:[631AF8]
// 004CE266 | 83F9 07       | CMP ECX,7
...
// z-axis
const int Z_AXIS_DEFAULT = 7; // default ground level // 004CE266 | 83F9 07 | CMP ECX,7
// 004CE260 | 8B0D F81A6300 | MOV ECX,DWORD PTR DS:[631AF8]
// 004CE266 | 83F9 07       | CMP ECX,7
...
// level spy
const int LEVELSPY_NOP_DEFAULT = 49451; // integer 2 bytes // 004CE270 | 2BC1 | SUB EAX,ECX

const int LEVELSPY_ABOVE_DEFAULT = 7; // integer 1 byte // 004CE26B | B8 07 | MOV EAX,7
const int LEVELSPY_BELOW_DEFAULT = 2; // integer 1 byte // 004CE273 | B8 02 | MOV EAX,2

const int LEVELSPY_MIN = 0;
const int LEVELSPY_MAX = 7;

// name spy
const int NAMESPY_NOP_DEFAULT = 19061; // integer 2 bytes // 004E95F9 | 75 4A | JNZ SHORT 004E9645
const int NAMESPY_NOP2_DEFAULT = 16501; // integer 2 bytes // 004E9603 | 75 40 | JNZ SHORT 004E9645

// show invisible creatures
const int INVISIBLE1_DEFAULT = 114; // integer 1 byte // 0045BF63 | 72 0A | JB SHORT 0045BF6F
const int INVISIBLE2_DEFAULT = 117; // integer 1 byte // 004E88C5 | 75 09 | JNZ SHORT 004E88D0

const int INVISIBLE_PATCH = 235; // integer 1 byte // 235 = EB = JMP // change JB and JNZ to JMP

// full light
const int LIGHT_NOP_DEFAULT = 1406; // integer 2 bytes // 004E1DA9 | 7E 05 | JLE SHORT 004E1DB0

const int LIGHT_AMOUNT_DEFAULT = 128; // integer 1 byte // 004E1DAB | BB 80 | MOV EBX,80
const int LIGHT_AMOUNT_FULL = 255; // integer 1 byte // 004E1DAB | BB 80 | MOV EBX,80
...
void doTrayIconCreate(HWND hwnd)
{
	...
	strcpy(trayIcon.szTip, "Level Spy - Tibia 8.22");
...
void doTrayMenuCreate(HWND hwnd)
{
	...
	AppendMenu(trayMenu, MF_STRING, ID_TRAY_MENU_NULL,				"Level Spy - Tibia 8.22");
...
void onCreate(HWND hwnd)
{
	...
	// loaded
	setStatusbarText("Level Spy for Tibia 8.22 loaded!");
}
...
void onDestroy(HWND hwnd)
{
	...
	// unloaded
	setStatusbarText("Level Spy for Tibia 8.22 unloaded!");
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR cmdline, int cmdshow)
{
	...
	HWND applicationWindow = FindWindow(0, "Level Spy - Tibia 8.22");
	...
	hwnd = CreateWindowEx(
		...
		"levelspy", "Level Spy - Tibia 8.22", // window class and title
