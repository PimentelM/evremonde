...
int main(int argc, char *argv[])
{
    ...
    char user_file_text_name[128];
    sprintf(user_file_text_name, "profiles/%s.txt", argv[1]);

    FILE *user_file_text = fopen(user_file_text_name, "w");
    if (user_file_text == NULL)
    {
        printf("Error: File \"%s\" could not be opened.\n", user_file_text_name);
        return 1;
    }

    fprintf(user_file_text, "Profile:\n%s\n", argv[1]);
    fprintf(user_file_text, "\n");
    ...
    fprintf(user_file_text, "Steam Community URL:\n%s\n", url_steam_community);
    ...
    fprintf(user_file_text, "TF2 Backpack Examiner URL:\n%s\n", url_tf2items);
    ...
    while (tfitems_item)
    {
        ...
        for (i = 0; i < cJSON_GetArraySize(tf2items_items); i++)
        {
            ...
            if (tf2items_item_id == tfitems_item_defIndex)
            {
                ...
                fprintf(user_file_text, "%s", tf2items_item_name);
    ...
    fprintf(user_file_text, "Summary:\n");
    fprintf(user_file_text, "--------\n");

    fprintf(user_file_text, "Items:       %d\n", nItems);
    fprintf(user_file_text, "Hats:        %d\n", nHats);
    fprintf(user_file_text, "Metals:      %d metal(s) worth %d item(s) or %d random hat(s) or %d class-specific hat(s)\n", nMetals, nMetalsWorth, (int)floor(nMetalsWorth / TF2_CRAFT_RANDOM_HAT_ITEM_REQUIREMENT), (int)floor(nMetalsWorth / TF2_CRAFT_CLASS_SPECIFIC_HAT_ITEM_REQUIREMENT));
    fprintf(user_file_text, "Tokens:      %d\n", nTokens);
    fprintf(user_file_text, "Total:       %d / %d\n", nItems + nHats + nMetals + nTokens, TF2_BACKPACK_MAX_SIZE);
    fprintf(user_file_text, "\n");
    fprintf(user_file_text, "Items Found: %d\n", nItemsFound);
    fprintf(user_file_text, "Hats Found:  %d\n", nHatsFound);
    fprintf(user_file_text, "Total Found: %d\n", nItemsFound + nHatsFound);
