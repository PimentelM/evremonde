...
void writeString(DWORD address, char* text)
{
	...
	{
		WriteProcessMemory(processHandle, (LPVOID)(address + j), &text[i], 1, 0);
		j++; // increment address
	}
...
// get window pointer
int getWindowPointer() { return readBytes(WINDOW_POINTER, 4); }

// get window caption
char* getWindowCaption()
{
	// get window caption
	return readString(getWindowPointer() + OFFSET_WINDOW_CAPTION);
}
...
{
	// get window visible
	return readBytes(getWindowPointer() + OFFSET_WINDOW_VISIBLE, 1);
}
...
void doFastHands()
{
	...
	{
		// check if window is the move objects window
		if (strcmp(getWindowCaption(), "Move Objects") == 0)
		{
...
void onCreate(HWND hwnd)
{
	...
	// loaded
	setStatusbarText("Fast Hands for Tibia 8.1 loaded!");
}

void onDestroy(HWND hwnd)
{
	// unloaded
	setStatusbarText("Fast Hands for Tibia 8.1 unloaded!");
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR cmdline, int cmdshow)
{
	...
	HWND applicationWindow = FindWindow(0, "Fast Hands - Tibia 8.1");
