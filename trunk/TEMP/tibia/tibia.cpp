...
void CTibia::doPlayerWalkTo(int toX, int toY, int toZ)
{
	// set the player's walking-to coordinates
	setPlayerGoTo(toX, toY, toZ);
...
{
	Trainer.writeBytes(HOTKEYS_OBJECT_BEGIN + (STEP_HOTKEY_OBJECT * (hotkeyNumber - 1)), itemId, 2);
	Trainer.writeBytes(HOTKEYS_OBJECT_BEGIN + (STEP_HOTKEY_OBJECT * (hotkeyNumber - 1)) + OFFSET_HOTKEY_OBJECT_TYPE, hotkeyType, 1);
}
...
void CTibia::setFullLight(bool enabled)
{
	if (enabled)
	{
		// enable full light
...
void CTibia::setLevelSpy(bool enabled)
{
	if (enabled)
	{
		// enable level spying
...
void CTibia::setNameSpy(bool enabled)
{
	if (enabled)
	{
		// enable name spying
...
void CTibia::setShowInvisibleCreatures(bool enabled)
{
	if (enabled)
	{
...
void CTibia::setFpsLimit(unsigned char* fps) { Trainer.writeArray(getFpsPointer() + OFFSET_FPS_LIMIT, fps, 8); }
...
void CTibia::setPlayerGoTo(int x, int y, int z)
{
	Trainer.writeBytes(PLAYER_GO_TO_X, x, 2);
	Trainer.writeBytes(PLAYER_GO_TO_Y, y, 2);
	Trainer.writeBytes(PLAYER_GO_TO_Z, z, 1);
}
...
void CTibia::setPlayerIsWalking(bool enabled) { setPlayerBattleList(OFFSET_CREATURE_IS_WALKING, enabled, 1); }
...
void CTibia::setTargetOutfit(Outfit_t outfitId) { setTargetBattleList(OFFSET_CREATURE_OUTFIT, outfitId, 2); }
...
void CTibia::setTargetSkull(Skull_t skullId) { setTargetBattleList(OFFSET_CREATURE_SKULL, skullId, 1); }

/* get */

int CTibia::getHotkeyType(int hotkeyNumber) { return Trainer.readBytes(HOTKEYS_TYPE_BEGIN + (STEP_HOTKEY_TYPE * (hotkeyNumber - 1)), 1); }

int CTibia::getHotkeyByObject(int itemId, HotkeyType_t hotkeyType)
{
	// search through hotkeys
	int j = 1; // starting hotkey number
	for(int i = HOTKEYS_OBJECT_BEGIN; i < HOTKEYS_OBJECT_END; i += STEP_HOTKEY_OBJECT)
	{
...
std::string CTibia::getStatusbarText() { return Trainer.readString(STATUSBAR_TEXT); }
...
int CTibia::getClickCount() { return Trainer.readBytes(CLICK_COUNT, 2); }
