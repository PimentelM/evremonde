...
CTibia::CTibia()
{
	...
	// set tibia to low cpu usage; causes lag when using multi-client
...
bool CTibia::isInGame() { return Trainer.readBytes(IS_INGAME, 1) == INGAME_YES; }
...
bool CTibia::isLevelSpyEnabled()
{
    return Trainer.readBytes(LEVELSPY_NOP, 2) != LEVELSPY_NOP_DEFAULT;
}
...
void CTibia::doReplaceMapObject(int oldObjectId, int newObjectId)
{
	...
	for(int i = mapBegin; i < mapBegin + (STEP_MAP_TILE * MAX_MAP_TILES); i += STEP_MAP_TILE)
	{
		for (int k = 0; k < (MAX_MAP_OBJECT_STEPS + 1); k++)
		{
...
{
	// trees array; hard-coded object ids for Tibia 8.0!
	int treesArray[] =
	{
