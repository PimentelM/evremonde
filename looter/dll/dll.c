...
void looterWriteInventoryToFile()
{
    ...
    for (i = 0; i < TIBIA_MAX_CONTAINERS; i++)
    {
        ...
        cJSON_AddNumberToObject(container, "isOpen",    _TIBIA_CONTAINERS->container[i].isOpen);
        cJSON_AddNumberToObject(container, "id",        _TIBIA_CONTAINERS->container[i].id);
        cJSON_AddNumberToObject(container, "unknown1",  _TIBIA_CONTAINERS->container[i].unknown1);
        cJSON_AddNumberToObject(container, "unknown2",  _TIBIA_CONTAINERS->container[i].unknown2);
        cJSON_AddStringToObject(container, "name",      _TIBIA_CONTAINERS->container[i].name);
        cJSON_AddNumberToObject(container, "volume",    _TIBIA_CONTAINERS->container[i].volume);
        cJSON_AddNumberToObject(container, "hasParent", _TIBIA_CONTAINERS->container[i].hasParent);
        cJSON_AddNumberToObject(container, "amount",    _TIBIA_CONTAINERS->container[i].amount);
        ...
        for (j = 0; j < TIBIA_MAX_CONTAINER_ITEMS; j++)
        {
            ...
            cJSON_AddNumberToObject(item, "id",      _TIBIA_CONTAINERS->container[i].item[j].id);
            cJSON_AddNumberToObject(item, "count",   _TIBIA_CONTAINERS->container[i].item[j].count);
            cJSON_AddNumberToObject(item, "unknown", _TIBIA_CONTAINERS->container[i].item[j].unknown);
        }
    }
...
// find container item
tibiaFindContainerItem_s looterFindContainerItem(int itemId, int itemCount, int startContainer, int startPosition, char* containerName, int shouldSkipMaxStackedItems)
{
    tibiaFindContainerItem_s containerItem;
    ...
    for(i = startContainer; i < TIBIA_MAX_CONTAINERS; i++)
    {
        ...
        if (_TIBIA_CONTAINERS->container[i].isOpen == 0)
            continue;

        // container must not be empty
        if (_TIBIA_CONTAINERS->container[i].amount == 0)
            continue;
        ...
        if (strlen(containerName) > 0)
        {
            ...
            if (strstr(_TIBIA_CONTAINERS->container[i].name, containerName) == NULL)
                continue;
        }

        // loop through container's items
        int j = 0;
        for (j = startPosition; j < _TIBIA_CONTAINERS->container[i].amount; j++)
        {
            // item id must exist
            if (_TIBIA_CONTAINERS->container[i].item[j].id == 0)
                continue;
            ...
            if (shouldSkipMaxStackedItems == 1)
            {
                ...
                if (_TIBIA_CONTAINERS->container[i].item[j].count >= TIBIA_MAX_ITEM_STACK)
                    continue;
            }

            // item ids must match
            if (_TIBIA_CONTAINERS->container[i].item[j].id == itemId)
            {
                containerItem.id            = _TIBIA_CONTAINERS->container[i].item[j].id;
                //containerItem.count       = _TIBIA_CONTAINERS->container[i].item[j].count;
                containerItem.container     = i;
                containerItem.containerName = _TIBIA_CONTAINERS->container[i].name;
...
void looterMoveContainerItemToEquipmentSlot(int itemId, char* fromContainerName, tibiaEquipmentSlot_t toEquipmentSlot)
{
    tibiaFindContainerItem_s containerItem = looterFindContainerItem(itemId, 0, 0, 0, fromContainerName, 0);
...
{
    tibiaFindContainerItem_s firstContainerItem = looterFindContainerItem(itemId, 0, 0, 0, "", 1);
    tibiaFindContainerItem_s nextContainerItem  = looterFindContainerItem(itemId, 0, firstContainerItem.container, firstContainerItem.position + 1, "", 1);
...
{
    tibiaFindContainerItem_s containerItem = looterFindContainerItem(itemId, 0, 0, 0, "", 0);
    if (containerItem.isFound == 0)
        return;
...
void looterLootItemsAroundPlayer(int itemId, tibiaEquipmentSlot_t toEquipmentSlot)
{
    tibia_move_object(*_TIBIA_PLAYER_X,     *_TIBIA_PLAYER_Y,     *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X - 1, *_TIBIA_PLAYER_Y - 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X + 1, *_TIBIA_PLAYER_Y + 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X - 1, *_TIBIA_PLAYER_Y + 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X + 1, *_TIBIA_PLAYER_Y - 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X - 1, *_TIBIA_PLAYER_Y,     *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X + 1, *_TIBIA_PLAYER_Y,     *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X,     *_TIBIA_PLAYER_Y - 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
    tibia_move_object(*_TIBIA_PLAYER_X,     *_TIBIA_PLAYER_Y + 1, *_TIBIA_PLAYER_Z, itemId, 1, 0xFFFF, toEquipmentSlot, 0x0, TIBIA_MAX_ITEM_STACK);
}

// parse items
void looterParseItems(looterParseItems_t parseItemsType)
{
    ...
    while (item)
    {
        ...
        tibiaEquipmentSlot_t toEquipmentSlot = TIBIA_EQUIPMENT_SLOT_BACKPACK; // default to backpack equipment slot
        ...
        if (parseItemsType == LOOTER_PARSE_ITEMS_LOOT || parseItemsType == LOOTER_PARSE_ITEMS_LOOT_ITEMS_AROUND_PLAYER)
        {
            if (itemIsLootEnabled == 1)
            {
                // check if player has enough cap or if container is full
                if (*_TIBIA_PLAYER_CAP == 0 || *_TIBIA_PLAYER_CAP < 1 || strstr(_TIBIA_STATUSBAR_TEXT, "You cannot put more objects in this container.") != NULL)
                {
                    // go to next item
                    item = item->next;
                ...
                if (parseItemsType == LOOTER_PARSE_ITEMS_LOOT)
                    looterMoveContainerItemToEquipmentSlot(itemId, "Dead", toEquipmentSlot);
                else if (parseItemsType == LOOTER_PARSE_ITEMS_LOOT_ITEMS_AROUND_PLAYER)
                    looterLootItemsAroundPlayer(itemId, toEquipmentSlot);
            }
        }

        // stack
        if (parseItemsType == LOOTER_PARSE_ITEMS_STACK)
            if (itemIsStackEnabled == 1)
                looterStackContainerItems(itemId);

        // use
        if (parseItemsType == LOOTER_PARSE_ITEMS_USE)
        {
            if (itemIsUseEnabled == 1)
            {
                // do not use food if player is full
                if (strcmp(itemType, "food") == 0 && strstr(_TIBIA_STATUSBAR_TEXT, "You are full.") != NULL)
                {
                    // go to next item
                    item = item->next;
...
void looterShowItemId()
{
    ...
    if (strstr(_TIBIA_SEE_TEXT, "You see"))
        strcpy(buffer, _TIBIA_SEE_TEXT);
    else
        strcpy(buffer, "...");

    strcat(buffer, " (Item ID: ");
    char itemId[128];
    itoa(*_TIBIA_SEE_ID, itemId, 10);
...
{
    looterParseItems(LOOTER_PARSE_ITEMS_LOOT);
}
...
{
    looterParseItems(LOOTER_PARSE_ITEMS_STACK);
}

// use
void timerUse(HWND hwnd)
{
    looterParseItems(LOOTER_PARSE_ITEMS_USE);
}
...
void onHotkey(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (wparam)
    {
        ...
        case ID_HOTKEY_LOOT_ITEMS_AROUND_PLAYER:
            looterParseItems(LOOTER_PARSE_ITEMS_LOOT_ITEMS_AROUND_PLAYER);
            break;
