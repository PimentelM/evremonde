...
const int INVISIBLE_PATCH = 235; // integer 1 byte; EB = JMP = 235; change JB and JNZ to JMP
...
void doShowInvisibleCreaturesToggle()
{
	...
	if ((invisible1 == INVISIBLE1_DEFAULT) || (invisible2 == INVISIBLE2_DEFAULT))
	{
		...
		// set statusbar
		setStatusbar("Show Invisible Creatures: On");
	}
	else
	{
		// disable show invisible creatures
		...
		// set statusbar
		setStatusbar("Show Invisible Creatures: Off");
	}
}
